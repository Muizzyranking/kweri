...
import { getNodeError, nodeHasError, validateQuery } from "@/lib/query-engine/validator";
...
describe("validateQuery — field validation", () => {
  it("fails when field is empty", () => {
    const rule: QueryRule = { ...createRule(""), operator: "equals", value: "x" };
  ...
  it("fails for unknown field", () => {
    const rule: QueryRule = { ...createRule("unknownField"), operator: "equals", value: "x" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
...
describe("validateQuery — operator compatibility", () => {
  it("fails when using contains on a number field", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "contains", value: "5" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("fails when using regex on a number field", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "regex", value: "\\d+" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  });

  it("fails when using contains on a date field", () => {
    const rule: QueryRule = { ...createRule("createdAt"), operator: "contains", value: "2024" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  });

  it("allows equals on enum field", () => {
    const rule: QueryRule = { ...createRule("status"), operator: "equals", value: "active" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
...
describe("validateQuery — value validation", () => {
  it("fails when value is empty for equals", () => {
    const rule: QueryRule = { ...createRule("name"), operator: "equals", value: "" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("fails when number field has non-numeric value", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "equals", value: "abc" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("fails when date field has invalid date value", () => {
    const rule: QueryRule = { ...createRule("createdAt"), operator: "before", value: "not-a-date" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("accepts valid regex", () => {
    const rule: QueryRule = { ...createRule("name"), operator: "regex", value: "^[A-Z]" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(true);
  });

  it("fails when in_array has empty value", () => {
    const rule: QueryRule = { ...createRule("country"), operator: "in_array", value: "  ,  " };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
...
describe("validateQuery — between operator", () => {
  it("fails when both values missing", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "between", value: "", valueTo: "" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("fails when from >= to for numbers", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "between", value: "50", valueTo: "20" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(false);
  ...
  it("passes with valid number range", () => {
    const rule: QueryRule = { ...createRule("age"), operator: "between", value: "18", valueTo: "65" };
    const result = validateQuery(makeRoot(rule), USERS_SCHEMA);
    expect(result.valid).toBe(true);
  });
...
describe("validateQuery — nested groups", () => {
  ...
  it("validates all rules across nested groups", () => {
    const validRule: QueryRule = { ...createRule("name"), operator: "equals", value: "Ada" };
    const invalidRule: QueryRule = { ...createRule("age"), operator: "equals", value: "abc" };
  ...
  it("passes a fully valid nested tree", () => {
    const r1: QueryRule = { ...createRule("name"), operator: "contains", value: "Ada" };
    const r2: QueryRule = { ...createRule("age"), operator: "greater_than", value: "18" };
