    ...
    uploadSchema(json) {
      try {
        ...
        if (!parsed.name || typeof parsed.name !== "string") {
          return { success: false, error: 'Schema must have a "name" field' };
        }
    ...
    importJSON(json) {
      try {
        ...
        if (!parsed.root || typeof parsed.root !== "object") {
          return { success: false, error: "Invalid query JSON: missing root" };
        }
        ...
        if (!parsed.schemaName || !getSchemaByName(parsed.schemaName)) {
          return {
            success: false,
